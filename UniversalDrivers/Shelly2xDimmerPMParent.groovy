/**
 * Shelly Autoconf 2x Dimmer PM Parent
 *
 * Parent driver for dual-dimmer Shelly devices with power monitoring.
 * Examples: Shelly Pro Dimmer 2PM
 *
 * Architecture:
 *   - App creates parent device with component data values
 *   - Parent creates 2 dimmer children as driver-level children in initialize()
...
metadata {
  definition(name: 'Shelly Autoconf 2x Dimmer PM Parent', namespace: 'ShellyDeviceManager', author: 'Daniel Winks', singleThreaded: false, importUrl: '') {
    capability 'Light'
 ...
 * Expected: 2 dimmer children, 0-4 input children.
 */
 ...
 *
 * @param dst The webhook destination (e.g., 'light_on', 'input_toggle')
 * @return The Shelly component type (e.g., 'light', 'input')
 */
...
/**
 * Builds events from webhook GET parameters for light, input, and power monitoring.
 *
...
private List<Map> buildWebhookEvents(String dst, Map params) {
  ...
  switch (dst) {
    // Discrete light webhooks — state encoded in dst name
    case 'light_on':
      ...
      if (params.brightness != null) {
        ...
        events.add([name: 'level', value: level, unit: '%',
    ...
    // Legacy combined light monitoring webhook
    case 'lightmon':
    ...
    // Temperature webhook (internal device temp sensor)
    case 'temperature':
...
/**
 * Handles on() command from child dimmer.
 *
...
/**
 * Handles off() command from child dimmer.
 *
 * @param childDevice The child device requesting the command
 */
...
/**
 * Handles setLevel() command from child dimmer.
 *
 * @param childDevice The child device requesting the command
...
/**
 * Handles startLevelChange() command from child dimmer.
 *
 * @param childDevice The child device requesting the command
...
/**
 * Handles stopLevelChange() command from child dimmer.
 *
 * @param childDevice The child device requesting the command
 */
...
/**
 * Sets brightness level on all dimmers.
 *
 * @param level Target brightness (0-100)
...
/**
 * Starts level change on all dimmers.
 *
 * @param direction Either "up" or "down"
 */
...
/**
 * Stops level change on all dimmers.
 */
...
/**
 * Resets energy monitoring counters for all dimmers.
 */
 ...
 * Stores per-component light state and recalculates parent aggregate.
 *
 * @param params The parsed webhook parameters
 */
private void processWebhookAggregation(Map params) {
  ...
  // Process light state aggregation
 ...
 *
 * @param componentType The component type (light, input, etc.)
...
private void updateAggregatesFromStatus(Map deviceStatus) {
  ...
  deviceStatus.each { k, v ->
    ...
    if (key.startsWith('light:') && v instanceof Map) {
      if (v.output != null) {
        lightStates[key] = v.output
      }
      if (v.brightness != null) {
        lightLevels[key] = v.brightness as Integer
      }
      if (v.apower != null) {
