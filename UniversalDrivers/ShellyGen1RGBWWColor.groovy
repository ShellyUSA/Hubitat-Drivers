/**
 * Shelly Gen1 RGBWW Color (SHRGBWW-01)
 *
 * Pre-built standalone driver for the Gen 1 Shelly RGBWW in color mode.
 ...
 *
 * The RGBWW uses the {@code /color/0} endpoint for commands, unlike bulbs which
...
metadata {
  definition(name: 'Shelly Gen1 RGBWW Color', namespace: 'ShellyDeviceManager', author: 'Daniel Winks', singleThreaded: false, importUrl: '') {
    capability 'Switch'
...
/**
 * Parses incoming LAN messages from the Gen 1 Shelly RGBWW.
...
/**
 * Routes Gen 1 action URL callbacks for RGBWW color events.
...
/**
 * Turns the RGBWW on via the /color/0 endpoint.
 */
void on() {
  logDebug('on() called')
...
/**
 * Turns the RGBWW off via the /color/0 endpoint.
 */
void off() {
  logDebug('off() called')
...
/**
 * Sets the RGBWW brightness (gain) level.
 * In color mode, the RGBWW uses 'gain' for brightness control.
 *
...
/**
 * Sets the RGBWW color using a Hubitat HSV color map.
...
/**
 * Sets the RGBWW hue, preserving current saturation and level.
 *
...
/**
 * Sets the RGBWW saturation, preserving current hue and level.
 *
...
/**
 * Distributes polled status data to the RGBWW device.
 ...
 * Uses /settings/color/0 endpoint for RGBWW color mode settings.
 */
