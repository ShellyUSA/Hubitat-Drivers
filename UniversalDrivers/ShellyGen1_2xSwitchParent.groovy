/**
 * Shelly Gen1 2x Switch Parent
 *
 * Parent driver for Gen 1 dual-relay Shelly devices without power monitoring.
 * Examples: Shelly 2 (SHSW-21) in relay mode
 *
 * Architecture:
...
metadata {
  definition(name: 'Shelly Gen1 2x Switch Parent', namespace: 'ShellyDeviceManager', author: 'Daniel Winks', singleThreaded: false, importUrl: '') {
    capability 'Switch'
 ...
 * Expected: 2 switch children (non-PM).
 */
void reconcileChildDevices() {
  ...
  components.each { String comp ->
    ...
    String driverName = 'Shelly Autoconf Switch'
...
private void routeActionUrlCallback(Map params) {
  ...
  switch (dst) {
    ...
    case 'switch_off':
      ...
      break
    case 'input_short':
      Integer btnShort = componentId + 1
      events.add([name: 'pushed', value: btnShort, isStateChange: true, descriptionText: "Button ${btnShort} was pushed"])
      break
    case 'input_long':
      Integer btnLong = componentId + 1
      events.add([name: 'held', value: btnLong, isStateChange: true, descriptionText: "Button ${btnLong} was held"])
      break
    default:
