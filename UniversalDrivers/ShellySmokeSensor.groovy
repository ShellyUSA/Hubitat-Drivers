/**
 * Shelly Autoconf Smoke Sensor
 *
 * Pre-built standalone driver for Shelly Gen 2+ smoke sensor devices.
 * Examples: Shelly Plus Smoke
 *
 * This is a battery-powered sensor device that sleeps most of the time.
 * It wakes briefly to send smoke/temperature updates via webhooks.
 * When a sensor notification arrives, the driver also captures piggybacked
 * battery data while the device is awake.
 *
 * Capabilities:
 *   - SmokeDetector: smoke attribute (clear/detected/tested)
 ...
 * Handles smoke alarm, temperature, and piggybacked battery data.
 *
 * Smoke webhook dst values:
 *   - smoke_alarm: smoke detected
 *   - smoke_alarm_off: smoke cleared
 *   - smoke_alarm_test: test button pressed
