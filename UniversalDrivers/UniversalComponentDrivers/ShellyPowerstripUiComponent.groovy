/**
 * Shelly Autoconf PowerstripUI - Component Driver
 *
 * Controls the LED indicator strip on Shelly Power Strip Gen4 via POWERSTRIP_UI.SetConfig RPC.
 *
 * The LED strip supports three modes:
 *   - 'switch' — LEDs reflect the on/off state of each outlet
 *   - 'power'  — LEDs indicate power consumption level (color gradient)
 *   - 'off'    — LED strip is completely disabled
 *
...
metadata {
  ...
  preferences {
    ...
    input name: 'ledMode', type: 'enum', title: 'LED Strip Mode',
      description: 'How the LED strip indicates device status',
      options: ['switch':'Switch State (outlet on/off colors)', 'power':'Power Consumption (usage gradient)', 'off':'Off (LED disabled)'],
      defaultValue: 'switch', required: false
...
void updated() {
  ...
  parent?.componentPowerstripUiSetNightMode(device, nightModeConfig)
}

/**
 * Turns the LED strip on with the current LED mode and last-used color/brightness.
 * If ledMode preference is 'off', this is a no-op (use setLedMode to change mode).
 */
void on() {
  logDebug('on() called')
  String mode = settings.ledMode ?: 'switch'
  parent?.componentPowerstripUiOn(device, mode)
}

/**
 * Turns the LED strip off (disables all LEDs via leds.mode = "off").
 */
void off() {
  logDebug('off() called')
...
/**
 * Sets the LED strip brightness level.
 * Level 0 turns the strip off; any other level turns it on with the stored color.
 *
 * @param level Brightness percentage (0-100)
 * @param duration Ignored (POWERSTRIP_UI has no transition support)
 */
void setLevel(BigDecimal level, BigDecimal duration = null) {
  logDebug("setLevel(${level}, ${duration}) called")
  parent?.componentPowerstripUiSetLevel(device, level as Integer)
}

/**
 * Sets the LED strip color using hue, saturation, and level.
 * Only applies when ledMode is 'switch'.
 *
...
void setColor(Map colorMap) {
  ...
  parent?.componentPowerstripUiSetColor(device, colorMap)
}
